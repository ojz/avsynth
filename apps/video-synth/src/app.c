...
typedef struct App {
    ...
    int    frames;       /* for the fps readout */
    Uint64 fps_t0;
} App;
...
/* ---- the app on the shell (ROADMAP D14) ---- */

static void vsynth_destroy(void *self);

static void *vsynth_create(AppHost *host, int argc, char **argv)
{
    ...
    /* The shell made the window; this wraps it with the picture and the
     * borderless drag, and puts it where the project last left it. */
    a->win = window_attach(host->window, host->renderer);
    if (!a->win) goto fail;
    window_set_geometry(a->win, win_x, win_y, win_w, win_h);
    a->hud = hud_create(host, &a->rack);
    if (!a->hud) goto fail;
    a->ed = editor_create(a->hud);
    a->help = help_create(a->hud);
    a->opt = options_create(a->hud);
    if (!a->ed || !a->help || !a->opt) goto fail;
    ...
    if (raw_vf) {
        ...
        if (restart_voice(a) < 0) goto fail;
        refresh_project_view(a);
    } else {
    ...
    fprintf(stderr,
        ...
        "  mouse: left-drag move, right-drag resize; panel: click row, drag fader, wheel nudges\n"
        ...
        "  c pick capture region, f fullscreen, F12 screenshot, q quit\n",
...
static void vsynth_destroy(void *self)
{
    ...
    if (a->voice) voice_stop(a->voice);
    options_destroy(a->opt);
    help_destroy(a->help);
    editor_destroy(a->ed);
    hud_destroy(a->hud);
    window_destroy(a->win);
    if (a->proj) project_close(a->proj);
    free(a);
}

/* Everything the shell did not take. For the mouse the shell went first and
 * took the fader hits; for the keyboard this runs first, so a text mode sees
 * every key. What is left falls to the shell: Tab, arrows, Backspace. */
static bool vsynth_event(void *self, const SDL_Event *e)
{
    App *a = self;
    SDL_Event ev = *e;
    ...
    if (ev.type == SDL_EVENT_KEY_DOWN) {
        ...
        if (k == SDLK_PAGEUP)   { step_chain(a, -1); return true; }
        if (k == SDLK_PAGEDOWN) { step_chain(a, +1); return true; }
    }

    /* the active overlay owns the keyboard; the mouse falls through to the window */
    if (a->mode == MODE_EDIT) {
        int act = editor_handle_event(a->ed, &ev);
        if (act == EDITOR_APPLY) { apply_editor(a); return true; }
        if (act == EDITOR_CLOSE) { set_mode(a, MODE_MAIN); show_status(a); return true; }
        if (act == EDITOR_CONSUMED) return true;
        if (ev.type == SDL_EVENT_KEY_DOWN) return true;   /* the editor's keyboard, even unused keys */
    } else if (a->mode == MODE_HELP) {
        int act = help_handle_event(a->help, &ev);
        if (act == HELP_CLOSE) { set_mode(a, a->help_from); return true; }   /* help is a detour */
        if (act == HELP_INSERT) {
            set_mode(a, MODE_EDIT);
        ...
        if (act == HELP_CONSUMED) return true;
    ...
    /* MAIN and PANEL share these keys. Tab, arrows and Backspace are not here
     * on purpose: they are the fader's, and the shell has them. */
    SDL_Keymod mod = SDL_GetModState();
    SDL_Keycode key = ev.key.key;
    int slot = slot_for_scancode(ev.key.scancode);
    if (slot) { preset_key(a, slot, (mod & SDL_KMOD_SHIFT) != 0); return true; }
    switch (key) {
    case SDLK_ESCAPE:
    ...
    case SDLK_E:
        ...
        return true;
    case SDLK_N:
        if (mod & SDL_KMOD_CTRL) { new_chain(a); return true; }
        return false;
    case SDLK_SPACE:
...
/* The shell moved a fader (param >= 0) or the selection (-1). */
static void vsynth_changed(void *self, int param)
{
    App *a = self;
    if (param >= 0) rack_send_control(&a->rack, a->voice, param);
    show_status(a);
}

static ParamSet *vsynth_params(void *self) { return &((App *)self)->rack.set; }
static const UiSurface *vsynth_surface(void *self) { return hud_surface(((App *)self)->hud); }

/* Once per loop: a failed voice opens the editor on its error; a new frame
 * and its taps go to their textures. */
static void vsynth_tick(void *self)
{
    App *a = self;
    ...
    if (f) {
        window_set_frame(a->win, f->data[0], f->width, f->height, f->linesize[0]);
        av_frame_free(&f);
        a->frames++;
        Uint64 now = SDL_GetTicks();
        if (now - a->fps_t0 >= 2000) {
            hud_set_fps(a->hud, a->frames * 1000.0 / (double)(now - a->fps_t0));
            a->frames = 0;
            a->fps_t0 = now;
        }
    }
...
static void vsynth_frame(void *self, SDL_Renderer *ren)
{
    App *a = self;
    window_draw(a->win);
    int w, h;
...
const AppSpec VSYNTH = {
    .name = "vsynth",
    .title = "vsynth",
    .window_w = 800,
    .window_h = 600,
    .window_flags = SDL_WINDOW_BORDERLESS,
    .create = vsynth_create,
    .destroy = vsynth_destroy,
    .event = vsynth_event,
    .tick = vsynth_tick,
    .frame = vsynth_frame,
    .params = vsynth_params,
    .surface = vsynth_surface,
    .changed = vsynth_changed,
};
